...
bool do_boolean_single(McutMesh &srcMesh, const McutMesh &cutMesh, const std::string &boolean_opts, const BooleanCancelCB& cancel_cb, const BooleanProgressCB& progress_cb)
{
    ...
    for (int n = 0; n < numConnComps; ++n) {
        ...
        // for each face in CC (we only rewrite indices in-place, so no local Vec3 container is needed)
        for (uint32_t f = 0; f < ccFaceCount; ++f) {
            if (cancel_cb && cancel_cb()) {
                return false;
            }
...
// Legacy overload kept for ABI compatibility with older callers that expect
// a void-returning do_boolean without callbacks.
void do_boolean(McutMesh &srcMesh, const McutMesh &cutMesh, const std::string &boolean_opts)
{
    // Ignore return value; legacy code did not check success.
    (void)do_boolean(srcMesh, cutMesh, boolean_opts, nullptr, nullptr, nullptr);
}
...
// Legacy overload kept for ABI compatibility with older callers that expect
// a 4-parameter make_boolean without callbacks.
...
{
    make_boolean(src_mesh, cut_mesh, dst_mesh, boolean_opts, nullptr, nullptr, nullptr);
}
