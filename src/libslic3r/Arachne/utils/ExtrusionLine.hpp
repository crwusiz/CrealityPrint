...
void extrusion_paths_append(std::list<ExtrusionPath>& dst, const ClipperLib_Z::Paths& extrusion_paths, const ExtrusionRole role, const Flow& flow, uint32_t perimeter_index, double overhang = 0);
void extrusion_paths_append(ExtrusionPaths &dst, const ClipperLib_Z::Paths &extrusion_paths, const ExtrusionRole role, const Flow &flow, uint32_t perimeter_index, double overhang = 0);
void extrusion_paths_append(ExtrusionPaths &dst, const Arachne::ExtrusionLine &extrusion, const ExtrusionRole role, const Flow &flow, uint32_t perimeter_index, double overhang = 0);
void extrusion_path_append(ExtrusionPaths& dst, const ThickPolyline& thick_polyline, const ExtrusionRole role, const Flow& flow,uint32_t perimeter_index, double overhang = 0);
} // namespace Slic3r
