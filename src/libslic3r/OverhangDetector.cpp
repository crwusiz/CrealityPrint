...
Polylines ZPath_to_polylines(const ZPaths& paths)
{
    ...
    for (auto& path : paths) {
        lines.emplace_back();
        for (auto& p : path)
            lines.back().points.push_back(Point{p.x(), p.y()});
    }
    return lines;
};
                                      ...
                                      const uint32_t             perimeter_index)
{
    ...
    ZPaths                   paths_in_range = clip_extrusion(extrusion_path, clip_paths,
                                                             ClipperLib_Z::ctIntersection); // doing intersection first would be faster.
    ...
    auto get_base_degree = [](double d) { return std::floor(d / min_degree_gap_arachne) * min_degree_gap_arachne; };
    ...
    auto get_split_points = [&](const Point& pa, const Point& pb, const coord_t wa, const coord_t wb, const double da,
                                const double db) -> std::vector<SplitPoint> {
    ...
    for (auto& path : paths_in_range) {
        ...
        for (size_t idx = 0; idx < path.size(); ++idx) {
            ...
            // map overhang speed to a range
            {
                ...
                if (it == non_uniform_degree_map.end()) {
                    ...
                    double t        = (degree - non_uniform_degree_map[low_idx]) /
                               (non_uniform_degree_map[high_idx] - non_uniform_degree_map[low_idx]);
        ...
        for (size_t idx = 1; idx < path.size(); ++idx) {
            ...
            if (in_same_degree_range(prev_d, curr_d)) {
                ...
                for (auto& split_point : split_points) {
                    ...
                    extrusion_paths_append(ret_path_list, {prev_line}, role, flow, perimeter_index, target_degree);
                    prev_line.clear();
                    prev_line.push_back({split_point.p.x(), split_point.p.y(), split_point.w});
                }
                prev_w = curr_w;
        ...
        if (prev_line.size() > 1) {
            extrusion_paths_append(ret_path_list, {prev_line}, role, flow, perimeter_index, get_base_degree(prev_d));
        }
    }
                            ...
                            const uint32_t       perimeter_index)
{
    ...
    for (size_t polyline_idx = 0; polyline_idx < middle_overhang_polyines.size(); ++polyline_idx) {
        ...
        for (SplitPoly& polylines_collection : out)
            extrusion_paths_append(paths, std::move(polylines_collection.polyline), polylines_collection.degree, int(0), role,
                                   extrusion_mm3_per_mm, extrusion_width, layer_height, perimeter_index);
        out.clear();
    }
}
} // namespace Slic3r